...
def test_collector_validates_market_data(tmp_path, monkeypatch):
    monkeypatch.setattr(microv5_loader, "fetch_lot_size_meta", lambda ex, sym: (0.0, 0.0, 0.0))
    # Simulate missing ticker price
    ex = DummyEx()
    monkeypatch.setattr(microv5_loader, "fetch_price_tick", lambda ex, sym: None)
    collector = MicroV5Collector(ex, "BTC/USDT", out_dir=tmp_path)
